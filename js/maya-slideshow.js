/** Eight Maya cards swap instantly in a staggered, high-variety stream. */
...
(function () {
    ...
    const CARD_COUNT = 8;
    const CHANGE_MS = 450;
    const DARK_PHASE = [true, false, false, true, true, false, true, false];
    let timers = [];
    ...
    class MayaStream {
        ...
        change(index) {
            ...
            this.indices[index] = (this.indices[index] + CARD_COUNT) % deck.length;
            this.active[index] = deck[this.indices[index]];
            this.dark[index] = this.active[index].kind === "generated" ? false : !this.dark[index];
            this.render(index);
        }
